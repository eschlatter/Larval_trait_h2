...
priorandpost_plot <- function(model,prior,xlim=0,i,xax=FALSE){
  ...
  if(xax==FALSE){
    plottt <- ggplot(as.data.frame(model$VCV), aes(x=as.data.frame(model$VCV)[,i])) + 
      theme_light()+
      geom_density(aes(color='Posterior'))+
      xlim(0,xlim)+
      geom_line(aes(x = xvals, 
...
herit_plot <- function(model,prior,xax=FALSE){
  herit = model$VCV[,1]/rowSums(as.data.frame(model$VCV))
  
  if(xax==FALSE){
    plottt <- ggplot(as.data.frame(herit), aes(x=as.data.frame(herit)[,1])) + 
      theme_light()+
      geom_density()+
      xlim(0,1)+
