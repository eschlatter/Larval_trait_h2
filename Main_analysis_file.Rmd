...
model_run <- function(param_list,model_statement,n_vars,n_mcmc_iter){
  ...
  #make a new scaled_vars dataframe, showing the proportional contribution of each source of variance to the total variance
  
  
#works, but doesn't output mcmc objects  
  # scaled_vars <- as.data.frame(model$VCV)
  # for(i in 1:ncol(model$VCV)){
  #   varname <- paste('scale',colnames(model$VCV)[i],sep='_')
  #   scaled_vars[[varname]] <- as.data.frame(model$VCV)[,i]/rowSums(as.data.frame(model$VCV))
  # }
  # scaled_vars <- dplyr::select(scaled_vars,starts_with('scale'))
  # scaled_vars <- rename(scaled_vars,herit=scale_animal)

  #outputs proportional variances as new mcmc objects
  scaled_vars <- list()
  for(i in 1:ncol(model$VCV)){
    varname <- paste('scale',colnames(model$VCV)[i],sep='_')
  ...
  #store results (mean, median, upper+lower95, effective sample size, last autocorr value)
  results=data.frame()
  for(i in 1:ncol(model$VCV)){
    v_name <- colnames(model$VCV)[i]
    v_mcmc <- model$VCV[,i]
    v_results <- data.frame(variable=v_name,
    ...
    results=rbind(results,v_results)
  }
  rownames(results)=colnames(model$VCV)
  
  results_long <- pivot_longer(results,mean:lastAutocorr,names_to='SummStat',values_to='Value') #in "long" format -- not sure yet which will be most useful

  
    
  #plot histograms of variance estimates
  mcmc_areas(scaled_vars,prob=0.95,area_method='scaled height') #scaled to 1
  
  

}

model_run(prior_invgamma2_params,"animal+clutch",2,1000)
```
