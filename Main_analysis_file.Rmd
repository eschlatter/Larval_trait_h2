...
model_results <- function(model){
  ...
  model_output <- list(diagnostic = dplyr::select(results,mean:lower95),estimates = dplyr::select(results,effSamp:lastAutocorr))
...
a <- model_run(prior_invgamma2_params,"animal+clutch",2,1000)
res <- model_results(a)
```
