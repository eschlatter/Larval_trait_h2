...
mcmc_iter=1000
model_run <- function(param_list,n_vars,n_mcmc_iter){
...
model <- MCMCglmm(length ~ 1+age,
                      random=formula(paste0('~',model_statement,sep='')),
                      prior=prior,
...
for(i in 1:ncol(model$VCV)){
  ...
  scaled_vars[[varname]] <- model$VCV[,i]/rowSums(model$VCV)
}
#scaled_vars <- mutate(scaled_vars,tot=scale_animal+scale_clutch+scale_units) test that the scaled columns add up to 1
...
#mcmc_areas(as.data.frame(model$VCV),prob=0.95,area_method='scaled height') #or in units of the phenotype (right? or squared units bc it's variance?)
